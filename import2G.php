                ...
                if ($sheetName == "Sites") {
                    foreach ($rows as $index => $row) {
                     ...
                     // Check if the site exists in NEW_SITES
                        ...
                        $rowExists1 = oci_fetch_assoc($checkStmt1);
        ...
        if ($sheetName == "2G Sites") {
            ...
            elseif ($sheetName == "3G Sites") {
                ...
                foreach ($rows as $index => $row) {
                    ...
                    $newsitecode = $siteCodeFromExcel .'U';
                    ...
                    if ($newSiteRow) {
                        ...
                        $checkSql3G = "SELECT SITE_ID FROM THREE_G_SITES WHERE SITE_CODE = :newsitecode";
                        $checkStmt3G = oci_parse($conn, $checkSql3G);
                        oci_bind_by_name($checkStmt3G, ":newsitecode", $newsitecode);
                        ...
                        if ($rowExists3G) {
                            ...
                            WHERE SITE_CODE = :newsitecode";
                            //echo $sid;
                            $updateresult2 = oci_parse($conn, $query);
                        ...
                        oci_bind_by_name($updateresult, ':lac', $row[12]);
                        
                        oci_bind_by_name($updateresult2, ':newsitecode', $newsitecode);
            ...
            elseif ($sheetName == "4G Sites") {
                ...
                foreach ($rows as $index => $row) {
                    if ($index == 0) continue; // Skip header row
                    $siteCodeFromExcel = $row[0];
                    $newsitecode = $siteCodeFromExcel .'L';
                    $checkSql1 = "SELECT ID ,SITE_CODE FROM NEW_SITES WHERE SITE_CODE = :sitecode";
                    $checkStmt1 = oci_parse($conn, $checkSql1);
                    oci_bind_by_name($checkStmt1, ":sitecode", $siteCodeFromExcel);
                    oci_execute($checkStmt1);
                    ...
                    if ($newSiteRow) {
                        ...
                        oci_bind_by_name($checkStmt4G, ":newsitecode", $newsitecode);
            ...
            elseif ($sheetName == "3G Cells") {
                ...
                foreach ($rows as $index => $row) {
                    ...
                    $checkSql2G = "SELECT SITE_ID FROM THREE_G_SITES WHERE SITE_CODE = :sitecode";
                    $checkStmt2G = oci_parse($conn, $checkSql2G);
                    oci_bind_by_name($checkStmt2G, ":sitecode", $siteCodeFromExcel);
                    oci_execute($checkStmt2G);
                    ...
                    if ($rowExists3G) {
                     ...
                     if($rowExists32){
                        ...
                        oci_bind_by_name($result2, ':aarea', $row[4]);
                        oci_bind_by_name($result2, ':earea', $row[5]);
                        ...
                        oci_bind_by_name($result2, ':car', $row[14]);
                        ...
                        $query2 = "INSERT INTO THREE_G_CELLS 
                                    (CID, MSCELL_ID,CELL_CODE, CELL_NAME, ON_AIR_DATE, AZIMUTH, M_TILT, E_TILT, TOTAL_TILT, SERVING_AREA, SERVING_AREA_IN_ENGLISH, NOTE, HIEGHT, CGI, CGI_HEX, CARRIER) 
                                    VALUES 
                                    (:cid,:ms,:cellcode, :cname, :celldate, :azimuth, :mtilt, :etilt, :ttilt, :aarea, :earea, :cnote, :height, :cgi, :hcgi, :car)";
            ...
            elseif ($sheetName == "4G Cells") {
                ...
                foreach ($rows as $index => $row) {
                    ...
                    $checkSql2G = "SELECT SID FROM FOUR_G_SITES WHERE SITE_CODE = :sitecode";
                    $checkStmt2G = oci_parse($conn, $checkSql2G);
                    oci_bind_by_name($checkStmt2G, ":sitecode", $siteCodeFromExcel);
                    oci_execute($checkStmt2G);
                    ...
                    if ($rowExists4G) {
                     ...
                     if($rowExists42){
                        ...
                        (CID_KEY,CELL_CODE, CELL_NAME, ON_AIR_DATE, AZIMUTH, M_TILT, E_TILT, TOTAL_TILT, SERVING_AREA, SERVING_AREA_IN_ENGLISH, NOTES, HEIGHT, CGI, CGI_HEX) 
                        VALUES 
                        (:cid, :cellcode, :cname, :celldate, :azimuth, :mtilt, :etilt, :ttilt, :aarea, :earea, :cnote, :height, :cgi, :hcgi)";
