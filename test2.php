...
input type ="text" name="cellid" size="4" placeholder="Cell ID" disabled value =
"<?= $data[$letter]['Cell_ID']?? '' //echo cell_id and if it is null echo ''?>">
...
SELECT t.Site_ID, t.Cell_ID, s.Site_code, s.Site_Name, t.Band ,s.Zone, s.Province, s.CR, s.Supplier, t.Site_Status,
s.Site_On_Air_Date, t.Notes, s.Coordinates_E, s.Coordinates_N, s.Site_Adress, s.BSC FROM new_sites s JOIN 2gsites t
ON(s.ID = t.Site_ID)
...
$query ="INSERT INTO `cancelled_sites`(`Site_ID`, `Cell_ID`, `Site_Code`, `Site_Name`, `Band`, `Zone`, `Province`, `CR`,
`Supplier`, `Status_Before`, `On_Air_Date`, `Cancellation_Date`, `Note`, `Coordinates_E`, `Coordinates_N`,
`Site_Adress`, `2G_RBS`) VALUES ($row['Site_ID'], $row['Cell_ID'], $row['Site_code'], $row['Site_Name'] , $row['Band']
,$row['Zone'] ,$row['Province'] ,$row['CR'], $row['Supplier'] , $row['Site_Status'] , $row['Site_On_Air_Date'] ,
$cancelldate , $row['Notes'], $row['Coordinates_E'], $row['Coordinates_N'] , $row['Site_Adress'] ,$row['BSC']) WHERE
s.ID = '$id'";